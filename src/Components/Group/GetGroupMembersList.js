...
export const GetGroupMembersList = ({ socket, ...props }) => {
  ...
  const handleSubmit = (event) => {
    ...
    socket.emit("get_group_members_list", values);
    setValues({ group_id: "" });
  };
  ...
  useEffect(() => {
    if (!isEmptyObject(socket)) {
      socket.on("group_members_list_response", (response) => {
        if (response.status) {
