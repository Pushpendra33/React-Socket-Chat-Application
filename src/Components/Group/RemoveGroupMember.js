...
export const RemoveGroupMembers = ({ socket, ...props }) => {
  ...
  useEffect(() => {
    if (!isEmptyObject(socket)) {
      socket.on("remove_group_member_response", (response) => {
        if (response.status) {
  ...
  const handleSubmit = (event) => {
    ...
    socket.emit("remove_group_member", values);
    ...
    <div className={"card my-3 mx-5 p-3"}>
      ...
      <form onSubmit={handleSubmit}>
        <div className="row">
          ...
          <div className={"col-6"}>
            <button className={"btn btn-info mt-4"}>Remove Group Member</button>
          </div>
        </div>
      </form>
    </div>
