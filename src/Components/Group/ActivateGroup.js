...
export const ActivateGroup = ({ socket, ...props }) => {
  ...
  const handleSubmit = (event) => {
    ...
    socket.emit("activate_group", values);
    setValues({ group_id: "" });
  };
  ...
  useEffect(() => {
    if (!isEmptyObject(socket)) {
      socket.on("activate_group_response", (response) => {
        if (response.status) {
    ...
    <div className={"card my-3 mx-5 p-3"}>
      ...
      <form onSubmit={handleSubmit}>
        <div className="row ">
          ...
          <div className={"col-6"}>
            <button className={"btn btn-info mt-4"}>Activate Group</button>
          </div>
        </div>
      </form>
    </div>
