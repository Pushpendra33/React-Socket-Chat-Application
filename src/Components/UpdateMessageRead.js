...
const UpdateMessageRead = ({ socket, ...props }) => {
  ...
  const handleSubmit = (event) => {
    ...
    socket.emit("update_message_read", values);
  };
  ...
  useEffect(() => {
    if (!isEmptyObject(socket)) {
      socket.on("update_message_read_response", (response) => {
        if (response.status) {
...
export default UpdateMessageRead;
